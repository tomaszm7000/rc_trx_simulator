...
root = tk.Tk()
root.title("RC Simulator for ESP32")
...
ttk.Button(root, text="Connect", command=connect_serial).grid(row=0, column=2, padx=5)

# Nagłówek
ttk.Label(root, text="Channels", font=("TkDefaultFont", 10, "bold")).grid(row=1, column=0, pady=(10, 0), sticky="w")

# CH1–CH3
for i, label in enumerate(["CH1 Steering", "CH2 Throttle/Brake", "CH3 Potentiometer"]):
...
# CH4 – przycisk
ttk.Label(root, text="CH4 Button").grid(row=5, column=0, sticky="w", padx=5)
ttk.Button(root, text="Press", command=toggle_button, width=10).grid(row=5, column=1, padx=5)
ttk.Label(root, textvariable=us_labels[3], width=7).grid(row=5, column=2, padx=5, sticky="e")

# CH5 i CH6
for idx, (ch, label, var) in enumerate([
    (4, "CH5 Switch", ch5_var),
    (5, "CH6 Switch", ch6_var)
]):
    row = 6 + idx
